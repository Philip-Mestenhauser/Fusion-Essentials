...
def _entity_type(obj) -> str:
    ...
    """
    if _safe(lambda: obj.isGroup):
        return "TimelineGroup"
    ent = _safe(lambda: obj.entity)
    if ent is None:
        return None
    try:
...
def _object_summary(obj) -> dict:
    health = _safe(lambda: obj.healthState)
    out = {
        "index": _safe(lambda: obj.index),
        "name": _safe(lambda: obj.name),
        "type": _entity_type(obj),
        "is_group": bool(_safe(lambda: obj.isGroup)),
        "is_suppressed": bool(_safe(lambda: obj.isSuppressed)),
        "is_rolled_back": bool(_safe(lambda: obj.isRolledBack)),
        "parent_group": _safe(lambda: obj.parentGroup.name if obj.parentGroup else None),
    ...
    msg = _safe(lambda: obj.errorOrWarningMessage)
    if msg:
        out["message"] = msg
    return out
...
def handler(include_suppressed: bool = True, group: str = "") -> dict:
    ...
    try:
        ...
        return _error(f"This design has no timeline (it may be a direct-modeling design, "
    ...
    try:
        ...
        return _error(f"Could not read the timeline: {e}")

    # Group roster: name -> member count, so the structure is visible at a glance.
    groups = {}
    try:
        for tg in timeline.timelineGroups:
            gname = _safe(lambda tg=tg: tg.name)
            if gname is not None:
                groups[gname] = _safe(lambda tg=tg: tg.count, 0)
    except Exception:
        pass

    payload = {
        "marker_position": _safe(lambda: timeline.markerPosition),
        "count": _safe(lambda: timeline.count),
        "returned": len(items),
        "groups": groups,
        "timeline": items,
    }
...
TOOL_DESCRIPTION = (
    ...
    "Read-only. Requires a parametric design with history (direct-modeling designs have "
    "no timeline)."
)
