...
old habit was to fish — design_get_tree, then assembly_probe, then cam_get_setups, then
design_get_timeline_health, then screenshots — each a round trip, each a guess about what's even
...
drill on demand. It composes signals the deeper tools expose (CAM detection à la cam_read, timeline
health à la design_ops, joint/ground rollup à la assembly_probe) into one digest — it does not replace
...
# Budget thresholds: above these, a whole-design dump (design_get_tree with no target, find_geometry
...
_BIG_OCCURRENCES = 40        # above this, design_get_tree(no target) is heavy -> suggest a target
...
def _data_identity(doc):
    ...
    (and has the URN that doc_copy / doc_open / data_* need) WITHOUT a separate doc_get_active_id call.
...
def _joint_rollup(root):
    """(joint_count, broken_joints[names]) — a joint is broken when healthState != 0 (the 'Compute
    Failed' a user sees). Same signal assembly_probe surfaces, rolled to a count + names here."""
    broken = []
...
def _browser_digest(root):
    ...
    exhaustive structure (that's design_get_tree's job, on demand)."""
    digest = []
    occs = safe(lambda: root.occurrences)
...
def handler() -> dict:
    ...
    "Switch to the Design workspace, or use the CAM tools if has_cam is true.")
        if out_of_date:
            note = (f"⚠ {len(out_of_date)} out-of-date reference(s) — run doc_update_xref. " + note)
        out["note"] = note
        return ok(out)
    ...
    pointers["assembly_structure"] = (
        f"design_get_tree(component='<name from browser_digest>') — {occ_total} occurrences is large; "
        "scope to a component rather than dumping the whole tree."
        if occ_total > _BIG_OCCURRENCES else
        "design_get_tree() — small enough to walk the whole assembly in one call.")
    pointers["geometry"] = (
    ...
    if has_cam:
        pointers["cam"] = ("cam_get_setups() / cam_get_operations() for the machining job; "
    ...
    # Lead the note with the single most important verdict (health), so an agent skimming the note
    # alone doesn't miss a broken design, then the progressive-disclosure guidance.
    if errors or broken_joints or out_of_date:
        bits = []
        if errors:
            bits.append(f"{errors} timeline error(s)")
        if broken_joints:
            bits.append(f"{len(broken_joints)} broken joint(s)")
        if out_of_date:
            bits.append(f"{len(out_of_date)} OUT-OF-DATE reference(s)")
        verdict = f"⚠ UNHEALTHY: {', '.join(bits)} — see health + the fix_* pointer(s) FIRST. "
    else:
        verdict = "Healthy (no timeline errors, broken joints, or stale references). "
    out["note"] = (
        verdict +
...
TOOL_DESCRIPTION = (
    "Cold-boot orientation — call FIRST on an open document for one cheap situational read instead of "
    "fishing across tool families. Returns the document + its data-model location (hub/project/folder + "
    ...
    "presence, and a 'pointers' block naming the targeted tool to refine each area (design_get_tree, "
    "find_geometry, assembly_probe, cam_get_setups). Orient here, then drill down with those scoped calls."
)
