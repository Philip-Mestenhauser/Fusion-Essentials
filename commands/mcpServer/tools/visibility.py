...
def _occ_state(occ) -> dict:
    return {
        ...
        "full_path": _safe(lambda: occ.fullPathName),
        "is_light_bulb_on": _safe(lambda: occ.isLightBulbOn),
        "is_visible": _safe(lambda: occ.isVisible),
        "is_isolated": _safe(lambda: occ.isIsolated),
    }
...
def handler(action: str = "", target: str = "") -> dict:
            ...
            for o in design.rootComponent.allOccurrences:
                if _safe(lambda o=o: o.isIsolated):
                    ...
                    try:
                        ...
                        return _error(f"Failed to clear isolation on '{before['name']}': {e}")
        except Exception as e:
            return _error(f"Could not scan for isolated occurrences: {e}")
        return _ok({"action": action, "cleared_count": len(cleared), "cleared": cleared})

    target = (target or "").strip()
    if not target:
        return _error(f"Provide 'target' — the occurrence name or full path to {action}.")
    ...
    if action == "isolate" and len(matches) > 1:
        names_hit = [(_safe(lambda o=o: o.fullPathName) or _safe(lambda o=o: o.name)) for o in matches]
        return _error(f"'{target}' matched {len(matches)} occurrences; isolate needs exactly "
    ...
    for o in matches:
        ...
        try:
            ...
            return _error(f"Failed to {action} '{before['name']}': {e}")
