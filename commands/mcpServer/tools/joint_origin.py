...
def _find_sketch(design, name):
    return _safe(lambda: design.rootComponent.sketches.itemByName(name)) if name else None
...
def _geometry_from_args(design, comp, anchor, target, x_cm, y_cm, z_cm,
    ...
    if anchor == "coordinates":
        ...
        g = _safe(lambda: JG.createByPoint(pt))
    ...
    if anchor == "sketch_line":
        ...
        g = _safe(lambda: JG.createByCurve(line, kp))
...
def handler(anchor: str = "coordinates", target: str = "at", units: str = "mm",
    ...
    if anchor not in _ANCHORS:
        return _error(f"Unknown anchor '{anchor}'. Valid: {', '.join(_ANCHORS)}.")

    target = (target or "at").strip().lower()
    if anchor == "coordinates" and target not in _TARGETS:
        return _error(f"Unknown target '{target}'. Valid: {', '.join(_TARGETS)}.")

    kp = (keypoint or "start").strip().lower()
    if kp not in _KEYPOINTS:
        return _error(f"Unknown keypoint '{keypoint}'. Valid: {', '.join(_KEYPOINTS)}.")
    ...
    if not jo_input:
        return _error("createInput returned nothing for this geometry.")

    # Resulting frame axes (Z primary, X secondary, Y third) — confirms orientation took.
    axes = {
        "primary_axis_Z": _vec(_safe(lambda: jo_input.primaryAxisVector)),
        "secondary_axis_X": _vec(_safe(lambda: jo_input.secondaryAxisVector)),
        "third_axis_Y": _vec(_safe(lambda: jo_input.thirdAxisVector)),
    }

    try:
        joint_origin = comp.jointOrigins.add(jo_input)
    except Exception as e:
        return _error(f"Joint origin creation failed: {e}")
    if not joint_origin:
        return _error("jointOrigins.add returned nothing.")
    ...
    payload = {
        "created": True,
        "joint_origin_name": _safe(lambda: joint_origin.name),
        "anchor": anchor,
        "anchored_on": desc,
        "frame_axes": axes,
...
TOOL_DESCRIPTION = (
    "Create a Joint Origin (a reusable coordinate frame / WCS anchor) in the active design, "
    "placed BY THE AGENT — no user click. The frame's orientation depends on what you anchor on:\n"
    "- anchor='coordinates' (default): position only at x,y,z (target='at', units mm/cm/in) or the "
