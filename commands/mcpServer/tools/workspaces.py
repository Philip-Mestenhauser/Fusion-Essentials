...
def list_workspaces_handler() -> dict:
    ...
    try:
        ...
        return _ok({"active_workspace": active, "workspace_count": len(workspaces),
                    "workspaces": workspaces})
    except Exception as e:
        return _error(f"Could not list workspaces: {e}")
...
def switch_workspace_handler(workspace: str = "") -> dict:
    ...
    if not want:
        return _error("Provide 'workspace' — an id, visible name, or alias "
    ...
    try:
        ...
        return _error(f"Could not enumerate workspaces: {e}")

    if not match:
        return _error(f"Workspace not found: '{workspace}'. "
                      f"Available: {', '.join(available) or '(none)'}")

    try:
        if match.isActive:
            return _ok({"switched": False, "active_workspace": match.name,
                        "note": "Workspace was already active."})
        ok = match.activate()
        if not ok:
            return _error(f"Activation of '{match.name}' failed (it may not be valid "
                          "to switch to right now, e.g. no document open).")
        return _ok({"switched": True, "active_workspace": match.name})
    except Exception as e:
        return _error(f"Failed to switch to '{match.name}': {e}")
...
list_workspaces_item = Item.create_tool_item(
    tool=_list_tool, handler=list_workspaces_handler, run_on_main_thread=True
)
...
switch_workspace_item = Item.create_tool_item(
    tool=_switch_tool, handler=switch_workspace_handler, run_on_main_thread=True
)
