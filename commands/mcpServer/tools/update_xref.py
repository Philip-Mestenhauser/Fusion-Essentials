...
def _ref_name(ref):
    return _safe(lambda: ref.dataFile.name) or "(unknown)"
...
def handler(name: str = "", only_out_of_date: bool = True) -> dict:
    ...
    if not doc:
        return _error("No active document.")

    refs = _safe(lambda: doc.documentReferences)
    count = _safe(lambda: refs.count, 0) if refs is not None else 0
    if not count:
        return _ok({"updated_count": 0, "updated": [], "note": "This document has no external references."})
    ...
    for i in range(count):
        ...
        if want and rname != want:
            continue
        matched += 1
        ood = bool(_safe(lambda ref=ref: ref.isOutOfDate, False))
        if only_out_of_date and not ood:
            skipped.append({"name": rname, "reason": "already up to date"})
            continue
        before_v = _safe(lambda ref=ref: ref.version)
        ok = _safe(lambda ref=ref: ref.getLatestVersion(), False)
        if not ok:
            errors.append({"name": rname, "error": "getLatestVersion returned false"})
            continue
        after_v = _safe(lambda ref=ref: ref.version)
    ...
    if want and matched == 0:
        ...
        return _error(f"No external reference named '{name}'. References in this document: "
                      f"{', '.join(available)}.")

    if errors:
        return _error(f"Some references failed to update: {json.dumps(errors)}. "
