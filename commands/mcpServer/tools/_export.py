...
MAP_BLURB = ("sanitize/verify_written/split_by_occurrence + find_component (the ONE design-wide "
             "by-name component resolve - (component, error), matched through "
             "_common._component_is_named so a spelling that resolves in one tool cannot miss in "
             "the next: a name SEVERAL components carry is REFUSED naming the occurrence "
             "fullPathNames that place them - instance_paths - and carries NO remedy, since only "
             "the caller knows which of its vocabularies still resolves; a miss is (None, None) "
             "the caller words itself, so no by-name component lookup exports the first of "
             "several) + top_level_occurrences "
             ...
             "STL_UNIT_MEMBERS + stl_unit_enum - the ONE unit key -> DistanceUnits member map every "
             "writer of an STL bakes its unitType from (NOT MeshUnits, whose mm/cm ints are "
             "swapped); applied_pair + NOT_APPLIED - the ONE export-options knob writer: it reads "
             "the property BEFORE the set as well and answers (landed value, changed), where "
             "'changed' is whether the ASSIGNMENT put the value there. A bare set-then-getattr is "
             "the shape that CANNOT BITE - measured, unitType reads 0 unset and "
             "MillimeterDistanceUnits IS 0, meshRefinement reads 1 unset and MeshRefinementMedium "
             "IS 1, and a boolean knob collides on whichever value the factory holds - so every "
             "knob writer goes through this and publishes 'changed' unless its read is MEASURED to "
             "determine the written file; pump_until - the shared "
