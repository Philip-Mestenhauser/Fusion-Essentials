...
def _ui():
    return _safe(lambda: app.userInterface)


# --------------------------------------------------------------- entity classification

def _component_of(entity):
    occ = _safe(lambda: entity.assemblyContext)
    if occ is not None:
        return _safe(lambda: occ.component.name), _safe(lambda: occ.fullPathName)
    comp = _safe(lambda: entity.parentComponent)
    if comp is not None:
        return _safe(lambda: comp.name), None
    body = _safe(lambda: entity.body)
    if body is not None:
        return _safe(lambda: body.parentComponent.name), None
    return None, None
...
def _xyz(pt):
    if pt is None:
        return None
    return {"x": round(_safe(lambda: pt.x, 0.0), 6),
            "y": round(_safe(lambda: pt.y, 0.0), 6),
            "z": round(_safe(lambda: pt.z, 0.0), 6)}
...
def _unit(vec):
    ...
    try:
        ...
        x, y, z = _safe(lambda: vec.x), _safe(lambda: vec.y), _safe(lambda: vec.z)
...
def _face_direction(face):
    ...
    """
    surf = _safe(lambda: face.geometry)
    stype = _safe(lambda: type(surf).__name__) if surf is not None else None
    if stype == "Plane":
        n = _safe(lambda: surf.normal)
        if n is None:
            # Evaluator fallback: normal at the face centroid.
            res = _safe(lambda: face.evaluator.getNormalAtPoint(face.centroid))
            if res and res[0]:
                n = res[1]
        return _unit(n), "face_normal"
    if stype in ("Cylinder", "Cone", "Torus"):
        return _unit(_safe(lambda: surf.axis)), "axis"
    if stype == "Sphere":
...
def _edge_direction(edge):
    ...
    crv = _safe(lambda: edge.geometry)
    ctype = _safe(lambda: type(crv).__name__) if crv is not None else None
    if ctype == "Line3D":
        sp = _safe(lambda: edge.startVertex.geometry)
        ep = _safe(lambda: edge.endVertex.geometry)
    ...
    if ctype in ("Circle3D", "Arc3D", "Ellipse3D"):
        ...
        return _unit(_safe(lambda: crv.normal)), "axis"
    return None, None


def _classify(entity) -> dict:
    """Structured description keyed off the entity's runtime type."""
    tname = _safe(lambda: type(entity).__name__) or "Unknown"
    ...
    if tname == "BRepFace":
        ...
        out.update({
            "kind": "face",
            "surface_type": _safe(lambda: type(entity.geometry).__name__),
            "area_cm2": round(_safe(lambda: entity.area, 0.0), 6),
            "centroid": _xyz(_safe(lambda: entity.centroid)),
            "direction": direction,        # planar -> normal; cyl/cone/torus -> axis (unit vec)
            "direction_kind": dir_kind,    # face_normal | axis | None
            "edge_count": _safe(lambda: entity.edges.count),
            "body_name": _safe(lambda: entity.body.name),
        ...
        out.update({
            "kind": "edge",
            "curve_type": _safe(lambda: type(entity.geometry).__name__),
            "length_cm": round(_safe(lambda: entity.length, 0.0), 6),
            "start": _xyz(_safe(lambda: entity.startVertex.geometry)),
            "end": _xyz(_safe(lambda: entity.endVertex.geometry)),
        ...
        out.update({
            "kind": "vertex",
            "position": _xyz(_safe(lambda: entity.geometry)),
            "body_name": _safe(lambda: entity.body.name),
            "component": comp, "component_path": path,
        })
    elif tname == "BRepBody":
        out.update({
            ...
            "is_solid": _safe(lambda: entity.isSolid),
            "volume_cm3": round(_safe(lambda: entity.volume, 0.0), 6),
            "area_cm2": round(_safe(lambda: entity.area, 0.0), 6),
            "component": _safe(lambda: entity.parentComponent.name),
            "component_path": _safe(lambda: entity.assemblyContext.fullPathName),
        })
    elif tname == "Occurrence":
        out.update({
            "kind": "component",
            "name": _safe(lambda: entity.name),
            "component_name": _safe(lambda: entity.component.name),
            "full_path": _safe(lambda: entity.fullPathName),
            "is_reference": _safe(lambda: entity.isReferencedComponent),
        })
    elif tname == "Component":
        out.update({"kind": "component", "name": _safe(lambda: entity.name)})
    else:
        out.update({"kind": "other", "name": _safe(lambda: entity.name)})
    return out
...
def request_user_selection_handler(what: str = "any", clear_current: bool = True) -> dict:
    ...
    ui = _ui()
    if not ui:
        return _error("No Fusion user interface available.")
    ...
    cleared = bool(_safe(lambda: ui.activeSelections.clear(), False)) if clear_current else None

    return _ok({
        "awaiting_user_selection": True,
        "requested_kind": kind,
        "cleared_previous_selection": cleared,
        "active_document": _safe(lambda: app.activeDocument.name),
...
def get_user_selection_handler(require: str = "") -> dict:
    ...
    sels = _safe(lambda: ui.activeSelections)
    ...
    try:
        for i in range(count):
            sel = sels.item(i)
            entity = _safe(lambda sel=sel: sel.entity)
            rec = _classify(entity) if entity is not None else {"object_type": None, "kind": "unknown"}
            rec["picked_point"] = _xyz(_safe(lambda sel=sel: sel.point))
            selections.append(rec)
    except Exception as e:
        return _error(f"Could not read the selection: {e}")
...
request_item = Item.create_tool_item(tool=request_tool, handler=request_user_selection_handler,
...
get_item = Item.create_tool_item(tool=get_tool, handler=get_user_selection_handler,
                                 run_on_main_thread=True)
