...
def _row_summary(row, active_id) -> dict:
    rid = _safe(lambda: row.id)
    return {
        "name": _safe(lambda: row.name),
        "id": rid,
        "index": _safe(lambda: row.index),
...
def _column_summary(col) -> dict:
    return {
        ...
        "title": _safe(lambda: col.title),
        "id": _safe(lambda: col.id),
        "index": _safe(lambda: col.index),
        "type": _safe(lambda: type(col).__name__),
    }
...
def _find_row(table, target):
    ...
    try:
        for r in rows:
            if (_safe(lambda r=r: r.name) or "") == target:
                return r
    except Exception:
        pass
    try:
        for r in rows:
            if (_safe(lambda r=r: r.id) or "") == target:
                return r
    except Exception:
        pass
    return None
...
def _collect(table) -> dict:
    ...
    active_row = _safe(lambda: table.activeRow)
    active_id = _safe(lambda: active_row.id) if active_row else None
    ...
    out = {
        "table_name": _safe(lambda: table.name),
        "table_id": _safe(lambda: table.id),
        "active_configuration": _safe(lambda: active_row.name) if active_row else None,
...
def handler(activate: str = "") -> dict:
    ...
    if not target:
        return _ok(_collect(table))

    # Activate a configuration by name/id.
    row = _find_row(table, target)
    if not row:
        available = [r.get("name") for r in _collect(table)["configurations"]]
        return _error(f"No configuration matched '{target}'. Available: "
                      f"{', '.join(str(a) for a in available)}.")

    before = _safe(lambda: table.activeRow.name)
    ok = _safe(lambda: row.activate(), False)
    if not ok:
        return _error(f"Activating configuration '{target}' failed (activate() returned false).")
