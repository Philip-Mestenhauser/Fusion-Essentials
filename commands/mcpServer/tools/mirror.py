...
def handler(bodies=None, plane: str = "yz", join: bool = False) -> dict:
    ...
    for b in body_ents:
        coll.add(b)
    if coll.count == 0:
        return _error("No valid bodies resolved to mirror.")
    ...
    try:
        ...
        return _error(f"Mirror failed: {e}.")
    if not feature:
        return _error("Mirror returned no feature.")

    result_bodies = []
    fb = _safe(lambda: feature.bodies)
    for i in range(_safe(lambda: fb.count, 0) if fb else 0):
        result_bodies.append(_safe(lambda i=i: fb.item(i).name))

    return _ok({
        "mirrored": True,
        "feature": _safe(lambda: feature.name),
        "source_bodies": [_safe(lambda b=b: b.name) for b in body_ents],
        "plane": (plane or "yz"),
        "joined": bool(join),
...
mirror_item = Item.create_tool_item(tool=mirror_tool, handler=handler, run_on_main_thread=True)
