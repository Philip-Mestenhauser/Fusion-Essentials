...
def handler(searchPattern: str = "", apiCategory: str = "all",
    ...
    try:
        ...
        return error(f"Invalid regex 'searchPattern': {e}")

    cat = (apiCategory or "all").strip().lower()
    if cat not in ("class", "member", "description", "all"):
        return error("apiCategory must be one of: class, member, description, all.")
    try:
    ...
    if not modules:
        return error(f"No API modules in scope for filter '{filter}'. Try 'adsk.core', "
...
api_doc_item = Item.create_tool_item(tool=api_doc_tool, write="read", handler=handler, run_on_main_thread=True)
