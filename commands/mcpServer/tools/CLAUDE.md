...
| `_doc_common` | _resolve_open_document - the open-document resolve doc_activate and doc_close share: one document from an 'open:N' index, a lineage URN / web URL matched by LINEAGE EQUALITY, or an exact display name, REFUSING more than one distinct match with the address each row reaches by; VERSION_LAG_WINDOW_S - the measured window a version read can still trail the cloud tip in |
