...
def _resolve_target(design, target):
    ...
    # Handle short-circuit: a find_geometry handle resolves a SPECIFIC body (bodies are auto-named, so a
    # handle is the precise path). Try the sanctioned resolver (composite-handle aware + self-healing)
    # FIRST; a plain name returns None and falls through to the by-name lookups below — no length guess.
...
def handler(target: str = "", frame: str = "", units: str = "mm") -> dict:
    ...
    if not design:
        return error("No active design (open a document with geometry).")
    ...
    if not entity:
        return error(f"Target not found: '{target}'. Provide a body or component/occurrence "
    ...
    "frame must be perpendicular, and the target must be B-Rep geometry.)")
        ...
        payload = {
            ...
            "x": round(safe(lambda: obb.length, 0.0) * f, 6),
...
TOOL_DESCRIPTION = (
    ...
    "measure here, then feed x/y/z into param_set."
)
