...
def get_cam_setups_handler() -> dict:
    ...
    try:
        ...
        "machine": safe(lambda: s.machine.name) if safe(lambda: s.machine) else None,
            ...
            "operation_count": safe(lambda: s.operations.count, 0),
            "total_operation_count": safe(lambda: s.allOperations.count, 0),
            })
    except Exception as e:
...
def _operation_summary(op) -> dict:
    ...
    # Surface the actual message text â€” a machinist reviewing toolpaths needs the content
...
def get_setup_references_handler(setup: str = "") -> dict:
    ...
    fusionWebURL â€” so the caller can `doc_open` the referenced fixture/part.
    """
    cam, err = _get_cam()
    if err:
        return error(err)
...
def activate_setup_handler(setup: str = "") -> dict:
    ...
    want = (setup or "").strip()
    if not want:
        return error("Provide 'setup' â€” the name of the setup to activate.")
    cam, err = _get_cam()
    if err:
        return error(err)
    ...
    try:
        for i in range(cam.setups.count):
            ...
            for op in safe(lambda: s.allOperations, []):
                ...
                entry["operations"].append(safe(lambda: operation.name))
                if s_name:
...
def get_machining_time_handler(setup: str = "") -> dict:
    ...
    # getMachiningTime(operations, feedScale, rapidFeed, toolChangeTime) â€” units confirmed live:
...
def get_nc_programs_handler() -> dict:
    ...
    are NOT exposed as readable post parameters on the NCProgram API (verified live â€”
    ...
    try:
        ...
        for i in range(ncs.count):
            ...
            "machine": safe(lambda: nc.machine.name) if safe(lambda: nc.machine) else None,
...
_setups_tool = Tool.create_simple(
    ...
    "workspace is Design (no need to switch to Manufacture). Returns an "
    "error if the document has no CAM data."
    ),
).strict_schema()
get_cam_setups_item = Item.create_tool_item(
    tool=_setups_tool, write="read", handler=get_cam_setups_handler, run_on_main_thread=True
)
...
_ops_tool = (
    Tool.create_simple(
        ...
        description=(
            ...
            "setup, or omit it for all setups. CAVEAT: the names/tools read fine "
        "anywhere, but the VALIDITY / is_out_of_date state is only trustworthy once the "
        "MANUFACTURE workspace has been entered â€” the CAM model does not re-evaluate against "
...
get_cam_operations_item = Item.create_tool_item(
    tool=_ops_tool, write="read", handler=get_cam_operations_handler, run_on_main_thread=True
)
...
_refs_tool = (
    Tool.create_simple(
        ...
        description=(
            "For each CAM setup, resolve its externally referenced (X-ref) components â€” "
        "among its selected models, fixtures, and stock â€” to the SOURCE document they "
...
get_setup_references_item = Item.create_tool_item(
    tool=_refs_tool, write="read", handler=get_setup_references_handler, run_on_main_thread=True
)
...
activate_setup_item = Item.create_tool_item(
    tool=_activate_tool, write="read", handler=activate_setup_handler, run_on_main_thread=True
)

_tool_list_tool = Tool.create_simple(
    name="sys_get_tool_list",
    description=(
    "List the distinct cutting tools used across the document's CAM operations â€” each "
...
get_tool_list_item = Item.create_tool_item(
    tool=_tool_list_tool, write="read", handler=get_tool_list_handler, run_on_main_thread=True
)
...
_time_tool = (
    Tool.create_simple(
        ...
        "rapid/tool-change assumptions. Pass 'setup' to limit to one setup. "
...
get_machining_time_item = Item.create_tool_item(
    tool=_time_tool, write="read", handler=get_machining_time_handler, run_on_main_thread=True
)
...
_nc_tool = Tool.create_simple(
    ...
    "post configuration, operation count, and the post parameters it exposes. Read-only â€” "
...
get_nc_programs_item = Item.create_tool_item(
    tool=_nc_tool, write="read", handler=get_nc_programs_handler, run_on_main_thread=True
)
...
_compare_tool = (
    Tool.create_with_string_input(
        ...
        "parameters differ â€” and the value on each side. Use this to understand what "
