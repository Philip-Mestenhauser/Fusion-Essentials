...
**rig-is-part-of-the-job**
When a setup is created: hold the fixture as its own components beside the part, select only the part body as the model, and take the work coordinate system from the stock; the fixture is geometry the toolpaths must avoid, not decoration. Except a setup sheet exercise with no fixture. Prove `cam_get`: selected_models names the part only and the wcs origin_mode is stated.

**steep-and-shallow-are-two-strategies**
When finishing a 3D form: parallel finishes shallow areas and skips steep ones by default; 3D contour does the opposite; one flag on each takes the other's areas, and scallop finishes both at a constant stepover a tenth of the tool. Except a flat part, where face and 2D contour suffice. Prove `cam_compare_operations`: machineSteepAreas or machineShallowAreas is the only difference between the pair.

**pocket-or-adaptive**
When roughing a pocket: adaptive takes deep stepdowns at a light radial load and needs no leads or compensation; 2D pocket takes shallow stepdowns with a finishing pass and cutter compensation - pick adaptive for bulk removal, pocket when the wall finish comes from the same operation. Except a slot, which wants the slot strategy on a closed slot contour. Prove `cam_compare_operations`: optimalLoad and maximumStepdown on adaptive; finishing passes and compensation on pocket.

**geometry-selection-is-half-the-operation**
When two operations read as identical: compare their geometry selections - chains, extension modes, stock contours, boundaries - because a parameter diff cannot see them. Except operations that differ in parameters already. Prove `cam_get`: each operation's references: the chains and faces it was given.
