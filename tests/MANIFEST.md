...
**Tools:** 134  |  **Input-kinds:** 15  |  write-status: `·` read · `✎` write · `⚠` destructive
...
| · | `model_create_holder` | Turn a solid HOLDER model into a CAM tool-holder profile — the headless form of the Add Tool Holder command |
...
| · | `model_measure_bbox` | Measure the bounding box of a target — a body or component/occurrence by name, or the whole design (omit 'target') |
...
| · | `model_physical_properties` | Compute the full PHYSICAL PROPERTIES of a target — mass (kg), volume, surface area, density, center of mass, the inertia tensor (Ixx…Ixz, about the world origin… |
...
| · | `mesh_measure` | Bounding box + triangle/vertex counts + watertight (is_closed) for ONE mesh body — the mesh analogue of model_measure_bbox, which can't see meshes |
...
| · | `cam_compare_operations` | Compare two CAM operations (by name) and report exactly which of their parameters differ â€” and the value on each side |
...
| ✎ | `cam_folder` | CAM FOLDERS in a setup |
...
| · | `cam_get_nc_programs` | List the document's NC programs (post/output jobs): each program's name, machine, post configuration, operation count, and the post parameters it exposes |
| · | `cam_get_operations` | Get the CAM operations in the active document, grouped by setup |
| · | `cam_get_references` | For each CAM setup, resolve its externally referenced (X-ref) components â€” among its selected models, fixtures, and stock â€” to the SOURCE document they come… |
| · | `cam_get_setups` | Get the CAM (Manufacture) setups in the active document: each setup's name, operation type (milling/turning/etc.), machine, the models/fixtures/stock it has sel… |
| · | `cam_get_status` | Poll a generation launched by cam_generate AND nudge it forward |
| · | `cam_get_time` | Estimate machining (cycle) time for the document's CAM program: per setup and total, with feed time, rapid time, and tool-change count |
| · | `cam_list_templates` | Navigate the CAM toolpath template library |
| ✎ | `cam_reorder` | REORDER a CAM operation/folder/pattern in the machining sequence: move 'entity' to 'before' or 'after' 'reference' (both are item names from cam_get_operations … |
...
| ✎ | `cam_tool_library` | Read & manage CAM TOOL LIBRARIES + their tools |
...
| ✎ | `design_configure` | BUILD a Configured Design (the write side of design_get_configurations) |
| ⚠ | `design_delete_feature` | Delete ONE timeline feature by name (from design_get_timeline) — e.g |
| ⚠ | `design_delete_occurrence` | Delete ONE component occurrence from the active design (e.g |
| ✎ | `design_export` | Export a body, component/occurrence, or the WHOLE design to a neutral CAD file on local disk (STEP / IGES / SAT / STL) |
| · | `design_get_configurations` | Read (and optionally switch) the configurations of the active Configured Design |
| · | `design_get_mode` | Report the active design's modeling MODE (parametric vs direct) and what you can do in it |
| · | `design_get_timeline` | Read the active parametric design's timeline — the ordered list of features, sketches, joints, occurrences, construction geometry, and groups that build the des… |
| · | `design_get_timeline_health` | Report the active design's parametric timeline health — feature error/warning rollup (names of any errored/warning features) |
| · | `design_get_tree` | Show the assembly structure (component/occurrence tree) of the active design, to a bounded depth |
...
| · | `doc_get_active_id` | Resolve the ACTIVE Fusion document to its data-model identity: its lineage id (URN) — the 'document_id' used by doc_copy and doc_open — plus version number, ope… |
...
| · | `doc_list_open` | List the documents open in the session: name, is_active, is_visible, is_saved, is_modified |
...
| ⚠ | `data_delete_folder` | Delete a data-model folder by its 'folder_id' (from data_list_folders) |
| ✎ | `data_hubs` | List the user's Autodesk data hubs, or attempt to SWITCH the active hub |
| · | `data_list_files` | List the files in a Fusion data project, identified by name (project) or id (project_id) |
| · | `data_list_folders` | Show the folder tree of a project, identified by 'project' (name) or 'project_id', to a bounded depth |
| · | `data_list_projects` | List the Fusion data projects in the user's active hub |
...
| | Tool | Summary |
|---|---|---|
| · | `workspace_orient` | Cold-boot orientation — call FIRST on an open document for one cheap situational read instead of fishing across tool families |
...
| · | `sys_get_tool_list` | List the distinct cutting tools used across the document's CAM operations â€” each tool's description (includes tool number, type, and geometry), how many opera… |
