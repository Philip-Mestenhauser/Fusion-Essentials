...
by measurement, not by hand; CONTRACTS.md is the human-readable ledger of the same run.
...
LEDGER = os.path.join(os.path.dirname(os.path.abspath(__file__)), "CONTRACTS.md")
...
def write_ledger(results, fusion_version, stamp_date):
    """Regenerate CONTRACTS.md from measurement results. results rows are (row, status, detail)."""
    lines = [
...
def check():
    ...
    if not os.path.exists(LEDGER):
        print("CONTRACTS.md does not exist - run measure_api.py once against live Fusion.")
        return 1
    with open(LEDGER, encoding="utf-8") as fh:
        text = fh.read()
    m = _STAMP_RE.search(text)
    if not m:
        print("CONTRACTS.md has no stamp line - regenerate it (run measure_api.py).")
        return 1
