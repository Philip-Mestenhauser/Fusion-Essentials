...
_CAM_SECOND_SETUP = [
    ...
    # The origin is a DESIGN write, and the CAM product learns of a design change only when the
    # Manufacture workspace re-activates (measured: a part-height edit left both flip operations
    # reading valid until this round trip flipped them out_of_date with "Design changed:
    # FlipSetup: Model"); a setup built on the stale product generated its silhouette contour
    # EMPTY. So the story steps out to Design and back before the flip setup is built.
    ("view_switch_workspace", {"workspace": "design"},
     lambda p: p.get("switched") is True and p.get("active_workspace") == "Design", None),
    ("view_switch_workspace", {"workspace": "manufacture"},
     lambda p: p.get("switched") is True and p.get("active_workspace") == "Manufacture", None),
    ...
    # BOTH flip operations generated BY NAME, and BEFORE the name dance below. A SETUP-level launch
    # of the pair left the never-generated contour at state valid with no toolpath and no fault
    # text in five receipt runs, while its own operation-level launch landed it every time; and a
    # rename starts the platform's own generation of the operation it names, so a path that
    # already exists is all the renames below regenerate.
    ("cam_generate", {"target": _FLIP_FACE_OP, "skip_valid": False},
     lambda p: p.get("launched") is True and p.get("target") == f"operation '{_FLIP_FACE_OP}'"
     and bool(p.get("handle")), None),
    ("cam_generate", {"target": _FLIP_BACK_OP, "skip_valid": False},
     lambda p: p.get("launched") is True and p.get("target") == f"operation '{_FLIP_BACK_OP}'"
     and bool(p.get("handle")), None),
    # Nothing below WRITES to either launched operation while its path lands: a rename landing
    # on the contour a beat after its launch cancelled the generation and left it valid with no
    # toolpath and no fault (five receipt runs). The name dance rides an operator note instead -
    # manual NC generates nothing, so a rename on it has nothing to cancel.
    ("cam_create_operation", {"setup": FLIP_SETUP, "strategy": "manual", "name": _FLIP_NOTE_OP,
                              "generate": False},
     _op_named(FLIP_SETUP, "manual", _FLIP_NOTE_OP), None),
    ...
    ("cam_edit_operation", {"operation": _FLIP_NOTE_OP, "rename": _POCKET_OP},
     _renamed_exactly(_FLIP_NOTE_OP, _POCKET_OP), None),
    ("cam_get", {"include": ["operations"], "setup": FLIP_SETUP},
     _setup_holds(FLIP_SETUP, _POCKET_OP), None),
    ("cam_get", {"include": ["operations"], "setup": CAM_SETUP},
     _setup_holds(CAM_SETUP, _POCKET_OP), None),
    ("cam_edit_operation", {"operation": _POCKET_OP + "#2", "rename": _FLIP_NOTE_OP},
     _renamed_exactly(_POCKET_OP, _FLIP_NOTE_OP), None),
    ("cam_get", {"include": ["operations"], "setup": FLIP_SETUP},
     _setup_holds(FLIP_SETUP, _FLIP_NOTE_OP), None),
    ("cam_get", {"include": ["operations"], "setup": CAM_SETUP},
     _setup_holds(CAM_SETUP, _POCKET_OP), None),
]
