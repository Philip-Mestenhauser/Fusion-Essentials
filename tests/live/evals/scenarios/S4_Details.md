...
ring pivot's TWO travel extremes (declare each ring pivot's travel range, drive across it, check at
the extremes, restore). The expected result is ZERO interfering pairs; the mounting holes and new
features must stay clear of the rings' FULL swing. Name any contact with its volume and the two
bodies; a hole or feature the rings swing into at an extreme is a FAIL.
...
- interference re-check ran at rest AND at each ring pivot's two travel extremes; the mounting
  holes and new features stay clear of the rings' full swing (zero interfering pairs, or any
  contact NAMED with its volume and the two bodies); report the checker's output at each pose.
...
- Staging: doc_open S3 artifact BY URN, confirm active; run the block. Budget:
  measured run + 25% (88 calls / 50.2k output tokens / 11.9 min,
  incl. a self-caught model_hole recovery; a PASS on a clean-mechanism P3).
