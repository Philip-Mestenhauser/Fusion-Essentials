...
| `S11_CAM-Extension` | CAM extension levers, cold and skill-less | doc_new + sketch/model_extrude (a drafted block), cam_create_setup, cam_edit_setup (machine), cam_edit_tools, cam_create_operation (swarf / a surface-driven 3D strategy / face), cam_select_geometry (chain -> swarf rails; surfaces + surface_target), cam_generate + cam_get_status, cam_get(include=['time']), cam_show_toolpath + view_screenshot, cam_post | three operations that CUT (non-zero machining time, none in the empty census); the swarf driven by the drafted wall; the 3-axis post subset landing with the multi-axis refusal reported; the wire alone teaching the rail/side/flute recipe; whether sys_get_guidance is ever called (diagnostic) |
