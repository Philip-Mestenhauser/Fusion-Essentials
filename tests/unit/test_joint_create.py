...
class TestJgFromEntity:
    def test_planar_face_center(self, monkeypatch):
        ...
        g, label, err = ji._jg_from_entity(Face(adsk.core.SurfaceTypes.PlaneSurfaceType))
    ...
    def test_cylinder_face_middle_not_center(self, monkeypatch):
        import adsk.core, adsk.fusion
        Face, _, _ = _jg_seam(monkeypatch)
        g, label, err = ji._jg_from_entity(Face(adsk.core.SurfaceTypes.CylinderSurfaceType))
    ...
    def test_circular_edge_center(self, monkeypatch):
        ...
        g, label, err = ji._jg_from_entity(Edge(adsk.core.Curve3DTypes.Circle3DCurveType))
    ...
    def test_line_edge_middle(self, monkeypatch):
        import adsk.core, adsk.fusion
        _, Edge, _ = _jg_seam(monkeypatch)
        g, _, err = ji._jg_from_entity(Edge(adsk.core.Curve3DTypes.Line3DCurveType))
    ...
    def test_vertex_uses_point(self, monkeypatch):
        ...
        g, label, err = ji._jg_from_entity(Vertex())
...
class TestCurrentJointType:
    ...
    def test_maps_each_motion_class(self):
        assert jn.current_joint_type(self._joint_with_motion("RevoluteJointMotion")) == "revolute"
        assert jn.current_joint_type(self._joint_with_motion("SliderJointMotion")) == "slider"
        assert jn.current_joint_type(self._joint_with_motion("CylindricalJointMotion")) == "cylindrical"
        assert jn.current_joint_type(self._joint_with_motion("PlanarJointMotion")) == "planar"
        assert jn.current_joint_type(self._joint_with_motion("RigidJointMotion")) == "rigid"
        assert jn.current_joint_type(self._joint_with_motion("BallJointMotion")) == "ball"

    def test_unknown_class_is_empty(self):
        assert jn.current_joint_type(self._joint_with_motion("MysteryJointMotion")) == ""

    def test_no_motion_is_empty(self):
        assert jn.current_joint_type(self._joint_with_motion(None)) == ""
...
class TestFaceExtentAndPlanar:
    ...
    def test_no_bbox_returns_zeros(self):
        ...
        assert ji._face_extent(f, 2) == (0.0, 0.0)
    ...
    def test_is_planar_true_only_for_surface_type_zero(self):
        ...
        assert ji._is_planar(planar) is True
        assert ji._is_planar(cyl) is False
