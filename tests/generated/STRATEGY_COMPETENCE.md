...
| manual | created | none | none - it takes no tool | ACT 10c7 ManualNC creates it and reads the name back, then deletes it - a generated Manual NC answers '3 : Machining time could not be calculated.' on its time row and never appears in empty_toolpaths, so neither non-empty oracle can judge it |
