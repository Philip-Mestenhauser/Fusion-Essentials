...
_TOOL_WEIGHTS = {
    ...
    "assembly_edit_relations": 2398,   # over P40: six actions over three relation kinds (rigid group / motion link / constraint), each with its own input, plus the measured set_occurrences refusal and the ratio-sign contract; peer design_edit_timeline 2164
    "assembly_edit_contacts": 2233,   # over P40: nine actions over one object - the set lifecycle (create with the >=2-distinct rule, set_members, rename, suppress, delete) plus the two design-level analysis flags whose truth table decides whether any set acts at all, and the enable-first fact the platform enforces by raising; peer assembly_edit_relations 2398
    ...
    "cam_generate": 1386,
    "cam_generate_setup_sheet": 1313,   # under the fleet P40 (1386) - add and go
    ...
    "cam_select_geometry": 3660,   # + the sketch and pocket_recognition selection kinds, per-kind input routing with refusals, loop/side/pocket-filter knobs read back, and the outputGeometry rung-3 read
    ...
    "data_download_file": 1579,   # over P40: the refusal contract (Fusion-native data leaves through design_export), the synchronous FREEZE warning, and the overwrite-removes-first semantics are each a fact a caller cannot recover after the call; peer data_upload_file 1577
    ...
    "design_add_instance": 2233,   # over P40: the placement surface + the landed-paths contract (children vs host instances named by measured cause); peers doc_insert_occurrence 2450, model_create_component 2231
    ...
    "design_configure": 3006,  # + add_material (per-configuration materials via the material theme table) and the add_configuration auto-activation disclosure
    ...
    "design_get": 2063,   # + the materials/appearances catalog slices (library census, scoped filtered pages) - in-family peers cam_get 3298, assembly_get 2323
    "design_recompute": 571,
    "design_remove_feature": 1289,   # under the fleet P40 (1386) - add and go
    "design_set_mode": 969,
    "doc_activate": 885,
    "doc_close": 1261,
    "doc_copy": 2091,
    ...
    "doc_save": 796,
    "doc_save_milestone": 1351,   # under the fleet P40 (1373) - add and go
    "doc_save_as": 1834,
    ...
    "joint_create": 3339,
    "joint_create_as_built": 1843,  # non-rigid motion: geometry anchor + joint_type/axis/slide_axis inputs, per-DOF pose pointers
    ...
    "mesh_plane_cut": 1452,
    "mesh_repair": 1631,   # five repair types + the six rebuild methods, each a typed Choice
    "mesh_reduce": 1280,
    "mesh_remesh": 822,
    ...
    "model_construction": 4414,   # three modes the API has no other route to (a plane rotated about a curved face's own axis, a plane pinned through a vertex, a plane/point placed along a path proportionally, absolutely, or to an object) plus the distance_type/to_object inputs they need
    "model_create_component": 2231,
    "model_draft": 2057,
    "model_emboss": 1341,   # under the fleet P40 (1373) - add and go
    "model_extrude": 3141,
    ...
    "model_inspect": 1566,
    "model_pipe": 2863,   # over P40: the path-fraction extents (both ends), section type/size, the order-coupled hollow wall verified off the feature, and the open-path refusal contract; nearest path-driven peer model_sweep
    ...
    "sketch_add_geometry": 4424,   # + the conic / cv_spline / elliptical_arc kinds (rho, degree, start_deg + the measured degree-clamp contract) and all four remaining slot constructors: two positional tail ladders with per-kind point roles (tips vs cap centre - the wrong reading mis-sizes every slot) and the tail-input refusals
    "sketch_constrain": 4417,   # 25 constraint kinds + per-instance pattern suppression (the rectangular row-column rule on the wire) + the four requested-only autoConstrain strategy knobs; 83 under the hard ceiling
    ...
    "sketch_dimension": 3515,   # ref vocabulary names ellipse/spline kinds; + the eight remaining SketchDimensions add* types, the surface operand and the driving/tangent-side flags, and the two measured dim_type behaviors a caller cannot see in the result (the angle wedge, the offset rotate)
    "sketch_edit_curve": 2118,   # seven curve-edit actions with per-curve pick points
    "sketch_insert_svg": 1363,   # under the fleet P40 - add and go
    "sketch_get": 1162,
    "sketch_project": 3558,   # two more actions on the same verb: to_surface (projectToSurface - faces, source curves from another sketch, project type, direction) and intersect (intersectWithSketchPlane - bodies/entities x the sketch plane); in-family peers sketch_constrain 3223, sketch_dimension 3515
    "sketch_set_text": 2885,   # + the along_path/fit_on_path modes (path, above_path, align, character_spacing), angle/flip formatting, and font_name (applies on create AND edit, landed/edited font read back) - in-family peers sketch_edit_curve 2118, sketch_constrain 3223
    ...
    "surface_fill": 1715,   # over P40: the cell-disclosure contract + two measured legality facts ARE the tool (peers surface_patch 1856)
    ...
    "sys_execute_script": 1655,   # the one-mutation-per-call rule with the measured rollback split (uncaught rolls back; caught is no guarantee)
    "sys_find_tool": 826,
