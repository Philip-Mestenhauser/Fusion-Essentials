...
_TOOL_WEIGHTS = {
    "appearance_set": 1697,   # +145: the base every override is copied from is named on the wire - measured, the color lands on a copy of the Fusion Appearance Library's 'Paint - Enamel Glossy (White)' (opaque, surface_roughness 0.07746), kept in the document as 'MCP Neutral Base'; a caller cannot recover which base its color rides on from the result, and the finish decides what the screenshot shows. + the measured transparency limit: opacity is refused below 255 because a Fusion appearance's transparency is its Prism material class, not its color alpha - a caller told only "opacity 0-255" reads the input as a transparency control the tool cannot deliver
    ...
    "assembly_edit_relations": 2837,   # over P40: six actions over three relation kinds (rigid group / motion link / constraint), each with its own input, plus the measured set_occurrences refusal and the ratio-sign contract; peer: design_edit_timeline
    ...
    "cam_get": 4507,   # + the setups slice reads the bound WCS back (origin/orientation mode + bound geometry), the only read-back of what cam_edit_setup's wcs binding did; paid for by trimming six description spots
    "cam_get_status": 1818,
    "cam_inspect_toolpaths": 1437,
    "cam_post": 3384,
    "cam_reorder": 1268,
    ...
    "design_export": 4805,   # 3MF/OBJ/USD/f3d/SMT formats, STL binary+units, DXF options, invisible flags
    ...
    "doc_insert_import": 2430,   # three targeting modes (component, plane for DXF, sketch for SVG) plus the sketch_insert_svg pointer telling a caller which of the two SVG routes places the art
    ...
    "joint_at_geometry": 2834,   # + the axis Choice enum (values machine-validated) and the frame-relative correction with its joint_edit(world_axis=) pointer; (ball uses none) stated on both surfaces
    "joint_create": 4278,
    ...
    "model_inspect": 2073,   # per_body names every occurrence in the subtree and discloses that a parent row aggregates its children - the read that stops a nested body folding invisibly into its parent
    ...
    "model_loft": 2387,   # +12 schema bytes, no prose: ProfileRefList's items now publish type ["string","object"], the two element forms it has always resolved. + is_closed (the one measured-real loft option; alignment measured a no-op on profiles and dropped) + the ordering sentence relocated from the ProfileRefList kind note
    ...
    "model_revolve": 2701,   # +20: the axis-defining face family named on the wire (cylindrical/conical/toroidal, each measured accepted), the fact a caller cannot recover from a refusal
    ...
    "model_stitch": 1907,
    "model_sweep": 2992,   # +70 for the measured chaining rule (tangent continuity, sharp corner stops it); +62 for the third declared output, path_curves - how many curves the built path HOLDS, the only number that shows a chain stopped short of the intended run
    ...
    "pmi_get": 2376,
    "save_as_mesh": 1473,
    "sketch_add_3d_line": 2202,
    "sketch_add_geometry": 5545,   # + the conic / cv_spline / elliptical_arc kinds (rho, degree, start_deg + the measured degree-clamp contract) and all four remaining slot constructors: two positional tail ladders with per-kind point roles (tips vs cap centre - the wrong reading mis-sizes every slot) and the tail-input refusals
    "sketch_constrain": 5851,   # 25 constraint kinds + per-instance pattern suppression (the rectangular row-column rule on the wire) + the four requested-only autoConstrain strategy knobs + the one entity-anchor mention (the ':center' form its point slots share with sketch_dimension) and the 'text:<i>' operand fix/unfix takes (the SketchText anchor DOF has no other route); paid for by a slimmer ref sentence, 449 under the hard ceiling
    "sketch_move": 2128,   # over P40: nine of the inputs ARE the transform (translation/rotation/scale composed into one matrix), verified by coordinate read-back
    "sketch_copy": 2186,   # over P40: same transform surface as sketch_move plus the target-sketch input and the new-refs contract
    ...
    "sketch_delete_entity": 1630,   # ref vocabulary names ellipse/spline kinds + the text:<index> target
    "sketch_dimension": 4060,   # ref vocabulary names ellipse/spline kinds; + the eight remaining SketchDimensions add* types, the surface operand and the driving/tangent-side flags, and the two measured dim_type behaviors a caller cannot see in the result (the angle wedge, the offset rotate)
    "sketch_edit_curve": 2730,   # seven curve-edit actions with per-curve pick points
    "sketch_insert_svg": 1870,   # over P40: the ignored width/height/viewBox with the 1/96-inch-times-scale rule, the y-down landing, and the doc_insert_import pointer are each a sizing/placement fact a caller cannot recover from the result
    "sketch_get": 1507,   # the read publishes 'frame' (world origin + X/Y/normal), the only typed route to a sketch plane's position, facing and coplanarity
    "sketch_project": 4188,   # two more actions on the same verb: to_surface (projectToSurface - faces, source curves from another sketch, project type, direction) and intersect (intersectWithSketchPlane - bodies/entities x the sketch plane); in-family peers: sketch_constrain, sketch_dimension
    "sketch_set_text": 3909,   # +129: 'height' states the create-only contract the handler now enforces - measured, an edit passing height=20 answered set:true/changed_count:1 with the text still at 8mm, because an edit writes the string and the font and nothing else; height is REFUSED at edit time now, which a caller cannot recover from a result that reported success. +54: 'y' names the VERTICAL anchor the same way 'x' names the horizontal one - measured, the requested y is the box's bottom and extra lines stack upward, and an agent reading only "Y position" placed a label by guessing the anchor and got it wrong. +70: 'x' names the anchor 'align' puts it on (left edge / center / right edge) - measured, the box moves with align and a caller cannot recover where centered text landed; + the along_path/fit_on_path modes (path, above_path, align, character_spacing), angle/flip formatting, and font_name (applies on create AND edit, landed/edited font read back) - in-family peers: sketch_edit_curve, sketch_constrain
    ...
    "view_section": 2596,
    "view_set": 4333,   # camera projection Choice + perspective angle with read-back; + the display action (the four browser-folder categories with their on/off flag)
