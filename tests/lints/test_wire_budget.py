...
_TOOL_WEIGHTS = {
    ...
    "drawing_update": 1274,
    "find_geometry": 2155,   # +170: a body inside a component resolves by name, and the qualified '<occurrence-or-component>:<body>' form the ambiguity refusal lists is what a caller must pass back - neither is recoverable from the schema
    ...
    "mesh_get": 1161,   # area/volume fields + units input
    ...
    "sketch_set_text": 2885,   # + the along_path/fit_on_path modes (path, above_path, align, character_spacing), angle/flip formatting, and font_name (applies on create AND edit, landed/edited font read back) - in-family peers: sketch_edit_curve, sketch_constrain
    ...
    "sys_execute_script": 1737,   # the one-mutation-per-call rule with the measured rollback split (uncaught rolls back; caught is no guarantee)
    "sys_find_tool": 826,
