...
shared map changes, and every copy the 2026 reading-audit found had already drifted to a different key
subset. `test_helper_duplication` only matched the exact name `OPERATIONS =`, so these leading-`_`
copies slipped through; this catches them by VALUE SHAPE: any dict literal outside _common whose values
name a `FeatureOperations` member (`...FeatureOperation`).
"""
