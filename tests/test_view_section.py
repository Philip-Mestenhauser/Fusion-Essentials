...
class TestPlaneCut:
    ...
    # REGRESSION: the DEFAULT call view_section(cut, plane='xy') runs with auto_view=True, which used
    # to read `pkey` in the bare-plane branch where it was never bound -> NameError AFTER the section
    # was created (error on the happy path). Every prior test masked it with auto_view=False.
